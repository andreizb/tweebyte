...
class RetweetControllerTest {
	...
	void testCreateRetweet() throws Exception {
		RetweetCreateRequest request = new RetweetCreateRequest().setOriginalTweetId(tweetId)
		...
		MvcResult result = mockMvc.perform(post("/retweets/{userId}", userId)
	...
	void testUpdateRetweet() throws Exception {
		...
		MvcResult result = mockMvc.perform(put("/retweets/{userId}/{retweetId}", userId, retweetId)
				.contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(request)))
				.andExpect(status().isOk())
				.andReturn();
	...
	void testDeleteRetweet() throws Exception {
		...
		MvcResult result = mockMvc.perform(delete("/retweets/{userId}/{retweetId}", userId, retweetId))
				.andExpect(status().isOk())
				.andReturn();
	...
	void testGetRetweetsByUser() throws Exception {
		List<RetweetDto> retweets = Collections
				.singletonList(new RetweetDto().setId(retweetId).setContent("User retweet"));
	...
	void testGetAllRetweetsOfTweet() throws Exception {
		List<RetweetDto> retweets = Collections
				.singletonList(new RetweetDto().setId(retweetId).setContent("Tweet retweet"));
