...
/**
 * Latency timers for the AI streaming surface. The end-to-end timer is split
 * by an {@code outcome} tag so success and error distributions can be analysed
 * separately downstream — a fast 5xx from {@code AbortPolicy} would otherwise
 * be averaged into the success p99 and mask the actual successful-request
 * latency cliff. Outcomes use the conventional Micrometer values
 * {@code success}, {@code error}, {@code cancel}.
 */
@Component
