 ...
 *
 * <ul>
 *   <li>{@code app.ai.backend=mock} (default): register {@link MockStreamingChatModel}
 *       as {@code @Primary}. Spring AI's OpenAI auto-config still runs but loses
 *       injection priority. Effectively: all traffic hits the calibrated mock.</li>
 *   <li>{@code app.ai.backend=live}: do not register the mock. Spring AI
 *       auto-configuration provides {@code OpenAiChatModel} from
 *       {@code spring.ai.openai.*} — the base-url can point at any
 *       OpenAI-compatible streaming endpoint (LM Studio, mlx_lm.server,
 *       Ollama, vLLM, ...). The {@code LIVE_LLM_BASE_URL} env var
 *       controls the endpoint; see infrastructure/compose/{async,reactive}.yml
 *       for the full passthrough.</li>
 * </ul>
 */
@Configuration
@Slf4j
public class AiConfiguration {
    ...
    /**
     * Startup safety check: shout loudly if the OpenAI base URL isn't pointing
     * at a local backend when AI_BACKEND=live. Prevents a typo from routing
     * benchmark traffic to api.openai.com with a real key.
     */
    @PostConstruct
