...
public class KeycloakClient {
	...
	// A hung Keycloak (DNS stall, dropped SYN, half-open socket) would otherwise park the
	// login/register/provisioning call forever; this caps every realm round-trip so the
	// caller fails fast (→ 504 via WebClient timeout) instead of pinning a connection.
	...
	public Mono<String> issueToken(String username, String password) {
		...
		return this.webClient.post()
			...
			.onStatus(HttpStatusCode::is4xxClientError,
					response -> Mono.error(new AuthenticationException("Invalid username or password")))
	...
	public Mono<Void> provisionUser(String email, String rawPassword, String userId) {
		return adminToken().flatMap(adminToken -> this.webClient.post()
			.uri("/admin/realms/{realm}/users", this.realm)
			.header(HttpHeaders.AUTHORIZATION, "Bearer " + adminToken)
			.bodyValue(userRepresentation(email, rawPassword, userId))
			.retrieve()
			.onStatus(HttpStatusCode::isError, response -> {
				log.warn("Keycloak user provisioning failed for {}: status {}", email, response.statusCode());
	...
	public Mono<Void> updateCredentials(String userId, String newEmail, String newRawPassword) {
		if (newEmail == null && newRawPassword == null) {
			return Mono.empty();
		}
		return adminToken().flatMap(adminToken -> findRealmUserId(adminToken, userId)
			.flatMap(realmId -> applyEmail(adminToken, realmId, newEmail)
				.then(applyPassword(adminToken, realmId, newRawPassword))))
			.timeout(KEYCLOAK_TIMEOUT);
	}
	...
	private Mono<String> findRealmUserId(String adminToken, String userId) {
		return this.webClient.get()
			...
			.header(HttpHeaders.AUTHORIZATION, "Bearer " + adminToken)
			.retrieve()
			.onStatus(HttpStatusCode::isError, response -> Mono.error(badGateway()))
			.bodyToMono(new org.springframework.core.ParameterizedTypeReference<List<RealmUser>>() {
			})
			.flatMap(users -> (users == null || users.isEmpty()) ? Mono.error(badGateway())
					: Mono.just(users.get(0).id()));
	}

	private Mono<Void> applyEmail(String adminToken, String realmId, String newEmail) {
		if (newEmail == null) {
			return Mono.empty();
		}
		...
		return this.webClient.put()
			...
			.header(HttpHeaders.AUTHORIZATION, "Bearer " + adminToken)
			.bodyValue(Map.of(USERNAME, newEmail, "email", newEmail))
			.retrieve()
			.onStatus(HttpStatusCode::isError, response -> Mono.error(badGateway()))
			.toBodilessEntity()
			.then();
	...
	private Mono<Void> applyPassword(String adminToken, String realmId, String newRawPassword) {
		...
		return this.webClient.put()
			...
			.header(HttpHeaders.AUTHORIZATION, "Bearer " + adminToken)
			.bodyValue(Map.of("type", "password", "value", newRawPassword, "temporary", false))
			.retrieve()
			.onStatus(HttpStatusCode::isError, response -> Mono.error(badGateway()))
			.toBodilessEntity()
			.then();
