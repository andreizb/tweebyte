 ...
 * subsequent post-tool ITL branch are actually exercised. Default test config leaves it
 * at 75 which never fires for tiny token streams.
 */
@ExtendWith(SpringExtension.class)
@WebFluxTest(controllers = AiController.class,
		excludeAutoConfiguration = { ReactiveSecurityAutoConfiguration.class,
				ReactiveUserDetailsServiceAutoConfiguration.class })
@Import(AiControllerToolInjectionTests.MockConfig.class)
@TestPropertySource(properties = "app.ai.tool-call-after-tokens=1")
class AiControllerToolInjectionTests {
	...
	void summarizeWithToolFiresInjectionAndExercisesPostToolItl() {
		...
		this.webTestClient.post()
			...
			.value(body -> {
				...
				// Tool event is injected as "[tool:alice]" and emitted alongside model
				// tokens.
				assertThat(body).as("expected tool event in stream: " + body).contains("[tool:alice]");
			});

		verify(this.aiMetrics, atLeastOnce()).recordTtft(any(Duration.class));
		verify(this.aiMetrics, atLeastOnce()).recordToolCall(any(Duration.class));
		// Post-tool tokens go through doOnNext where lastTokenNanos != -1L
		// and !token.startsWith("[tool:"), exercising the ITL branches.
		verify(this.aiMetrics, atLeastOnce()).recordItl(any(Duration.class));
	}
	...
	void summarizeReportsCancelOutcomeWhenClientCancels() {
		...
		this.webTestClient.post()
			.uri("/tweets/ai/users/" + UUID.randomUUID() + "/conversations/" + CONVERSATION_ID + "/summarize")
			.contentType(MediaType.APPLICATION_JSON)
