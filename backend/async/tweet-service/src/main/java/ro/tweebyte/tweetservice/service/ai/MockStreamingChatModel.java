...
/**
 * Spring AI {@link ChatModel} backed by calibrated synthetic distributions. TTFT sampled
 * from log-normal, inter-token intervals from gamma — via Apache Commons Math so the
 * distributions match the calibration module's fitted parameters exactly (K-S
 * comparable).
 *
 * <p>
 * Selected when {@code app.ai.backend=mock} (default). When {@code live}, Spring AI
 * auto-configuration provides {@link org.springframework.ai.openai.OpenAiChatModel}
