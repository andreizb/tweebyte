...
/**
 * Latency timers for the AI streaming surface. The end-to-end timer is split by an
 * {@code outcome} tag so success and error distributions can be analysed separately
 * downstream — a fast 5xx from {@code AbortPolicy} would otherwise be averaged into the
 * success p99 and mask the actual successful-request latency cliff. Outcomes use the
 * conventional Micrometer values {@code success}, {@code error}, {@code cancel}.
 *
...
public class AiLatencyMetrics {
	...
	/** Outcome tag value for a stream that completed normally. */
	public static final String OUTCOME_SUCCESS = "success";

	/** Outcome tag value for a stream that terminated with an error. */
	public static final String OUTCOME_ERROR = "error";

	/** Outcome tag value for a stream the client cancelled before completion. */
	...
	// Pre-register the three outcome variants of the e2e timer at startup so they
	// appear in /actuator/prometheus from the first scrape (Micrometer registry
	// also caches lookups, but eager registration avoids a missing-series race
	// for very-short-lived runs that hit only one outcome).
