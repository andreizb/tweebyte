	...
	void benchmarkProfileNeutralizesResilience4jAndDeclaresTheMeasuredKnobs() throws IOException {
		try (InputStream inputStream = getClass().getClassLoader()
			...
			assertThat(properties.getProperty("resilience4j.bulkhead.configs.default.maxConcurrentCalls"))
				.isEqualTo("2147483647");
			// AI streamExecutor reject policy (benchmark methodology, not a perf knob).
			assertThat(properties.getProperty("app.concurrency.stream.reject-policy")).isEqualTo("abort");

			// Measured benchmark knobs declared on the measured path: the DB pool auto-grows to
			// maximum-pool-size, and connection-timeout is effectively infinite so a thread waiting
			// for a connection queues instead of erroring (0% errors).
			assertThat(properties.getProperty("spring.datasource.hikari.maximum-pool-size")).isEqualTo("256");
			assertThat(properties.getProperty("spring.datasource.hikari.minimum-idle")).isEqualTo("10");
			assertThat(properties.getProperty("spring.datasource.hikari.connection-timeout")).isEqualTo("3600000");
