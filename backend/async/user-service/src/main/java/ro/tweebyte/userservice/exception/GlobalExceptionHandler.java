...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<Map<String, Object>> handleAuthenticationException(AuthenticationException ex,
			HttpServletRequest request) {
		return new ResponseEntity<>(getErrorsMap(List.of(ex.getMessage()), request, HttpStatus.UNAUTHORIZED),
	...
	public ResponseEntity<Map<String, Object>> handleRateLimit(RequestNotPermitted ex,
			HttpServletRequest request) {
		return new ResponseEntity<>(getErrorsMap(List.of(ex.getMessage()), request, HttpStatus.TOO_MANY_REQUESTS),
	...
	public ResponseEntity<Map<String, Object>> handleUser(UserException ex, HttpServletRequest request) {
		HttpStatus status = (ex.getStatus() != null) ? ex.getStatus() : HttpStatus.INTERNAL_SERVER_ERROR;
		if (status.is5xxServerError()) {
			log.error("Unhandled UserException on {}", request.getRequestURI(), ex);
		}
	...
	// A service-thrown ResponseStatusException carries its own status such as a bad
	// request, and without this handler the Throwable catch-all below would clobber
	// that status to an internal server error.
