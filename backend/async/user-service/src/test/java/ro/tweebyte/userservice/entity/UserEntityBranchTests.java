...
class UserEntityBranchTests {
	...
	void allArgsConstructorPopulatesAllFields() {
		// Async UserEntity field order (Lombok @AllArgsConstructor): id, userName, email,
		// biography, password, isPrivate, birthDate, createdAt, profilePictureId, isInsertable.
		...
		UserEntity entity = new UserEntity(id, "alice", "a@b", "bio", "pw", true, birth, createdAt, profilePictureId,
				true);
	...
	void differentInstancesAreNotIdentityEqual() {
		...
		assertThat(System.identityHashCode(b)).isNotEqualTo(System.identityHashCode(a));
	}
	...
	@Test
	void insertableFlagMarksEntityAsNew() {
		// Persistable contract: the register mapper sets this flag so save() does an INSERT.
		UserEntity entity = new UserEntity();
	...
	@Test
	void loadedEntityWithIdAndFlagClearedIsNotNew() {
		// The update path loads via findById (flag stays false) and keeps a non-null id, so
		// save() merges → UPDATE rather than INSERT.
		UserEntity entity = new UserEntity();
		entity.setId(UUID.randomUUID());
		entity.setInsertable(false);
