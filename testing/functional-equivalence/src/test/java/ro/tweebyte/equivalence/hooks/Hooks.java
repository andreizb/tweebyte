...
/**
 * Per-scenario lifecycle: clear DBs + scenario state before each scenario; dump body of
 * failed scenarios to the report on failure.
 */
public class Hooks {
	...
	public void onFailureDumpResponse(Scenario scenario) {
		...
		scenario.attach("Last HTTP status: " + ctx.lastStatus + "\nLast body:\n" + ctx.lastBody, "text/plain",
				"last-http-response");
	}
