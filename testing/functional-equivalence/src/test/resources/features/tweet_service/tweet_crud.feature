Feature: Tweet CRUD
  ...
  GET /tweet-service/tweets/user/{userId}, PUT /tweet-service/tweets/{userId}/{tweetId},
  DELETE /tweet-service/tweets/{userId}/{tweetId}.
  PUT/DELETE are owner-scoped: the {userId} segment must own {tweetId} or the
  request 404s. All gateway-routed paths require a Bearer JWT.
  ...
  Scenario: A user can edit their own tweet
    ...
    Then the response status is 200
    # Note: PUT /tweets/{userId}/{tweetId} returns 200 (not 204) on this codebase. Both stacks agree.
  ...
  Scenario: A user cannot update another user's tweet
    Given user "ian" is registered with email "ian@example.com" and password "Sup3rS3cret!"
    And user "ines" is registered with email "ines@example.com" and password "Sup3rS3cret!"
    When user "ian" posts tweet "ian owns this tweet content"
    Then the response status is 200
    When user "ines" attempts to update tweet of "ian" with content "ines tries to hijack this"
    Then the response status is 404

  Scenario: A user cannot delete another user's tweet
    Given user "igor" is registered with email "igor@example.com" and password "Sup3rS3cret!"
    And user "isla" is registered with email "isla@example.com" and password "Sup3rS3cret!"
    When user "igor" posts tweet "igor owns this tweet content"
    Then the response status is 200
    When user "isla" attempts to delete tweet of "igor"
    Then the response status is 404
    When user "igor" fetches tweet "igor owns this tweet content"
    Then the response status is 200
  ...
  # NOTE: DELETE is owner-scoped — it loads the tweet via findByIdAndUserId and
  # 404s when the row is missing or owned by another user (symmetric with PUT).
  # Both stacks agree on 404.
  Scenario: Deleting a non-existent tweet returns 404
