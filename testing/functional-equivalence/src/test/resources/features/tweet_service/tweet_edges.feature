Feature: Tweet-service edge cases (404s, malformed inputs, mention paths, cache hits)
  ...
  Scenario: Posting a tweet referencing an unknown media id returns 400
    # Drives validateMediaIds exists=false reject branch in TweetService
    # and the body != null && !exists path in UserClient.mediaExists.
    Given user "media-poster" is registered with email "media-poster@example.com" and password "Sup3rS3cret!"
    When user "media-poster" posts tweet with unknown media id
    Then the response status is 400

  Scenario: Posting a tweet with an explicit empty media id list succeeds
    Given user "media-empty" is registered with email "media-empty@example.com" and password "Sup3rS3cret!"
    When user "media-empty" posts tweet "tweet with explicit empty media list" with an empty media id list
    Then the response status is 200

  Scenario: Listing own tweets returns tweets with both hashtag and mention in page
    # Drives TweetService.splitRelations with both H and non-H (mention) rows in the
    # same enrichTweetsPage call — the getUserTweets paginated path, not the single-tweet
    # fetch path which uses enrichSingleTweetDto.
    Given user "split-rel-a" is registered with email "split-rel-a@example.com" and password "Sup3rS3cret!"
    And user "split-rel-b" is registered with email "split-rel-b@example.com" and password "Sup3rS3cret!"
    When user "split-rel-a" posts tweet "hey @split-rel-b check out #splitreltopic here"
    Then the response status is 200
    When user "split-rel-a" lists their own tweets
    Then the response status is 200

  Scenario: Keeping the same mention on update is a no-op for mention diff
    # Drives TweetUpdateService.reconcileMentions removeIf false branch:
    # desired.contains(m.getText()) is true so the predicate returns false and the
    # mention is kept rather than removed.
    Given user "keep-ment-a" is registered with email "keep-ment-a@example.com" and password "Sup3rS3cret!"
    And user "keep-ment-b" is registered with email "keep-ment-b@example.com" and password "Sup3rS3cret!"
    When user "keep-ment-a" posts tweet with single mention of "keep-ment-b"
    Then the response status is 200
    When user "keep-ment-a" updates their last tweet to mention "keep-ment-b"
    Then the response status is 200

  Scenario: Tweet media reference endpoint returns ids after a media-backed post
    Given user "tweet-mref" is registered with email "tweet-mref@example.com" and password "Sup3rS3cret!"
    When user "tweet-mref" uploads a 32x32 image
    Then the response status is 200
    When user "tweet-mref" posts tweet "tweet carrying one media reference" with the uploaded media id
    Then the response status is 200
    When user "tweet-mref" fetches tweet referenced media ids
    Then the response status is 200
