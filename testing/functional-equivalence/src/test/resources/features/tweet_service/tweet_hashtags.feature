Feature: Tweets with hashtags + mentions
  ...
  Scenario: Updating a tweet to add a brand-new hashtag creates it via reconcileHashtags supplier
    # Forces the computeIfAbsent insertion lambda in TweetUpdateService.reconcileHashtags.
    # The tag #xq7brandnewhashonly is unique across the suite so findByTextIn returns nothing
    # on the update path, causing the computeIfAbsent supplier to fire and create the row.
    # The tweet is created plain (no hashtags) so the CREATE path (HashtagService) never
    # seeds this tag first — the UPDATE path hits the supplier cold.
    Given user "newhash-u1" is registered with email "newhash-u1@example.com" and password "Sup3rS3cret!"
    When user "newhash-u1" posts tweet "plain tweet with no hashtags at all here"
    Then the response status is 200
    When user "newhash-u1" updates tweet "plain tweet with no hashtags at all here" content to "now adding #xq7brandnewhashonly that never existed before"
    Then the response status is 200
