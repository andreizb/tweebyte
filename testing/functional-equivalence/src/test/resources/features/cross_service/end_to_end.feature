Feature: End-to-end cross-service flows
  ...
  # B1 / US-2 — UserService.getUserSummaries empty-list guard (1 branch)
  # A brand-new user with zero followees causes interaction-service to call
  # getUserSummaries with an empty id list → early-return branch fires.

  Scenario: Following-cache fill with zero followees returns empty list
    Given user "efresh-a" is registered with email "efresh-a@x.com" and password "Sup3rS3cret!"
    When user "efresh-a" fetches follow recommendations
    Then the response status is 200
