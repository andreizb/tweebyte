Feature: Replies
  ...
  Scenario: Listing replies on a multi-replied tweet succeeds
    ...
    Then the response status is 200
    When user "pat" lists replies for tweet "tweet wanted by two repliers" expecting 2
    Then the response status is 200
  ...
  # Drives ReplyService.deleteReply's ownership guard — a non-author delete of an
  # EXISTING reply is forbidden: both stacks raise ResponseStatusException(FORBIDDEN)
  # -> 403, matching the retweet/like owner-only checks. (A missing reply stays 404.)
  ...
  # Drives ReplyService.updateReply's ownership guard — a non-author update of an
  # EXISTING reply is forbidden: both stacks raise ResponseStatusException(FORBIDDEN)
  # -> 403, matching the retweet/like owner-only checks. (A missing reply stays 404.)
