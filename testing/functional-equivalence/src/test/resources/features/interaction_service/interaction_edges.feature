Feature: Interaction-service edge cases (404s, no-ops, cache warm/hit)
  ...
  # Liking the same tweet twice is idempotent: the duplicate insert trips the
  # UNIQUE(user_id, likeable_id, likeable_type) constraint, which both stacks
  # swallow in LikeService.saveOrGetExisting and return the already-existing row
  # with 200. This drives that duplicate-key swallow branch in LikeService.
  Scenario: Liking the same tweet twice — second like is idempotent (200)
