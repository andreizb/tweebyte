Feature: Profile update edge cases (no-op variants, malformed inputs, 404s)
  ...
  # B1 / US-1 — UserService.updateUser duplicate-email/username + profilePictureId branches

  Scenario: Updating email to one already used by another user returns 400
    Given user "uconf-dup-email-a" is registered with email "uconf-dup-email-a@example.com" and password "Sup3rS3cret!"
    And user "uconf-dup-email-b" is registered with email "uconf-dup-email-b@example.com" and password "Sup3rS3cret!"
    When user "uconf-dup-email-a" attempts to update their email to "uconf-dup-email-b@example.com"
    Then the response status is 400

  Scenario: Updating userName to one already used by another user returns 400
    Given user "uconf-dup-name-a" is registered with email "uconf-dup-name-a@example.com" and password "Sup3rS3cret!"
    And user "uconf-dup-name-b" is registered with email "uconf-dup-name-b@example.com" and password "Sup3rS3cret!"
    When user "uconf-dup-name-a" attempts to update their userName to "uconf-dup-name-b"
    Then the response status is 400

  Scenario: Profile update with a non-existent profilePictureId returns 400
    Given user "uconf-badpic" is registered with email "uconf-badpic@example.com" and password "Sup3rS3cret!"
    When user "uconf-badpic" updates their profile picture to a non-existent id
    Then the response status is 400

  Scenario: Profile update with a valid profilePictureId succeeds
    Given user "uconf-goodpic" is registered with email "uconf-goodpic@example.com" and password "Sup3rS3cret!"
    When user "uconf-goodpic" uploads a 32x32 image
    Then the response status is 200
    And the response body has an id
    When user "uconf-goodpic" updates their profile picture to the uploaded media id
    Then the response status is 204
