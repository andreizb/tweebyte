Feature: User profile read + update
  ...
  Scenario: A user can delete their account
    ...
    # DELETE /users/{id} returns 204; the user row is removed and a follow-up read 404s.
    When user "leo" deletes their account
    Then the response status is 204
    When user "leo" fetches their own profile
    Then the response status is 404

  # DELETE /users/{id} is an ownership-enforced route: the caller's token must own
  # the path id. A random (non-owned) id is rejected at the gateway with 403 before
  # it can reach the service, so the deny lands as Forbidden rather than Not Found.
  Scenario: Deleting an account the caller does not own returns 403
    Given user "mia" is registered with email "mia@example.com" and password "Sup3rS3cret!"
    When user "mia" attempts to delete a non-existent account
