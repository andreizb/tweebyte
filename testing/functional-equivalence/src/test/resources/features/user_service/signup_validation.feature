Feature: User signup validation errors
  ...
  # B1 / UM-1 — UserMapper.mapRequestToEntity isPrivate explicit-false branch (1 branch)

  Scenario: Register with explicit isPrivate false succeeds and user is not private
    When a client registers user "signup-notprivate" with email "signup-notprivate@example.com" and password "Sup3rS3cret!" and isPrivate "false"
    Then the response status is 200
    And the response body contains a JWT token
