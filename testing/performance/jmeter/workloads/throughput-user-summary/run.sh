#!/usr/bin/env bash
# Bench runner for throughput-user-summary (GET /users/summary/{id}) — a pure
# single-service DB read (findById -> serialize).
#
# Usage:
...
WORKLOAD="throughput-user-summary"
BENCH_URL="http://localhost:9091"
# user-summary only reads user-service's own DB (findById); we still boot the whole
# stack for parity and wait on all three health endpoints.
HEALTH_URLS="http://localhost:9091/actuator/health http://localhost:9092/actuator/health http://localhost:9093/actuator/health"
# 1M cohort (prepare.py --seed-users 1 also seeds tweets/follows — unused by the
# findById read but harmless; reuses the user-profile cohort/seeder verbatim).
USER_COUNT=1000000
...
seed_and_verify() {
  echo "[$(date)] seeding user cohort (findById source for the summary read)..."
  python3 testing/performance/jmeter/workloads/throughput-user-summary/prepare.py \
...
run_stack() {
  ...
  BENCHMARK_TOPOLOGY="$TOPOLOGY" BENCHMARK_INFRA_TOPOLOGY="$INFRA_TOPOLOGY" \
    ...
    --user-ids-file testing/performance/jmeter/workloads/throughput-user-summary/payload/user-ids.csv \
