...
public class ValidateMockCommand implements Callable<Integer> {
    ...
    @CommandLine.Option(names = "--calibration", defaultValue = "testing/performance/k6/workloads/ai-stream-summarize/calibration/calibration.json",
    ...
    public Integer call() throws Exception {
        ...
        if ("gamma".equalsIgnoreCase(itlFamily)) {
            System.out.println("Note: For empirical samples with comb-quantized structure such as Qwen3.5-4B-MLX gap-mode ITL, no low-parameter smooth family — including the 3-parameter shifted log-normal and the 5-parameter 2-component log-normal mixture — has been observed to pass K-S at the calibration source's n. Treat the gamma fit as a first-order surrogate (mean preserved to ~0.03 %) suitable for E[response]-driven analysis; for distributional equivalence a comb-aware family or empirical-CDF inverse-transform sampler would be required (out of scope here).");
            System.out.println("Suggested diagnostics: (a) compare --itl-family={gamma, shifted_lognormal, lognormal_mixture} K-S statistics for the magnitude of the rejection, (b) inspect the empirical histogram.");
        } else {
            System.out.println("Suggested diagnostics: (a) compare --itl-family options to find the lowest K-S statistic, (b) inspect the empirical histogram — a comb-aware sampler may be needed for full distribution match.");
        }
        return 1;
    }
