...
#
# This script does NOT go in deployment/docker-compose/compose.sh. That script orchestrates
...
#
# Usage:
#   ./testing/performance/k6/workloads/ai-stream-summarize/realism-backend.sh up [--port 8081] [--parallel 8] [--model <path>]
#   ./testing/performance/k6/workloads/ai-stream-summarize/realism-backend.sh down
#   ./testing/performance/k6/workloads/ai-stream-summarize/realism-backend.sh status
#
# Once this script is up, point the SUT at it for a native-local live run with
# no Docker on the measured path. The `realism-live` runner
# profile sets AI_BACKEND/LIVE_LLM_* (and keeps LIVE_LLM_MAX_TOKENS consistent with the
# recorded tokens_per_response) for you:
#
#   ./testing/performance/k6/workloads/ai-stream-summarize/realism-backend.sh up
#   LIVE_LLM_BASE_URL=http://localhost:8081 \
#     LIVE_LLM_MODEL=/Users/andrei/.lmstudio/models/mlx-community/Qwen3.5-4B-MLX-4bit \
#     TOPOLOGY=native-local \
#     ./testing/performance/k6/workloads/ai-stream-summarize/run.sh realism-live both
#
# Heap: native-local has no compose `mem_limit`, so the runner's default `-Xmx4g`
# posture applies (AGENTS.md). The old Docker recipe (`-Xms1g -Xmx1500m
# -XX:+AlwaysPreTouch`, sized to dodge an 8 GB compose mem_limit) is OBSOLETE here —
# only narrow the heap as a clearly labelled live exception via
# AI_TWEET_JAVA_TOOL_OPTIONS if you need to free host RAM for mlx_lm.server.
...
VENV_DIR="${REALISM_BACKEND_VENV:-${REPO_ROOT}/deployment/.venv-realism}"
...
usage() {
  cat <<EOF
Usage:
  ./testing/performance/k6/workloads/ai-stream-summarize/realism-backend.sh up   [options]
  ./testing/performance/k6/workloads/ai-stream-summarize/realism-backend.sh down
  ./testing/performance/k6/workloads/ai-stream-summarize/realism-backend.sh status
...
Environment overrides:
  REALISM_BACKEND_VENV       venv path (default: ./deployment/.venv-realism)
...
cmd_up() {
  ...
  # Bind 0.0.0.0 so host-local clients reach it at localhost:PORT (the native-local
  # path) and, for the legacy all-Docker reproduction, containers via host.docker.internal.
