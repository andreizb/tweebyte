// Cross-service-fan-out GET /tweets/{tweetId} benchmark.
// This is the single-tweet READ workload for tweet-service: the backend
// enrichSingleTweetDto fans out per request to interaction-service (likes /
// replies / retweets counts + the replies list) and, via the replies list, to
// user-service (a getUserSummary per reply author), and reads the tweet's own
// hashtag + mention rows from tweet_service_db. Heavier than following-cache's
// pure Redis read: a hit still pays the four interaction-service HTTP hops.
//
// Mirrors following-cache: warmup + main constant-VU scenarios, a hot/cold key
// split, discardResponseBodies, and main_* counters/trends so the shared
// run_bench.sh parser (which keys off main_http_reqs / main_http_req_duration)
// reads the steady window identically. The only request-shape difference is the
// URL: ${PATH_PREFIX}/${id} (PATH_PREFIX=/tweets) instead of /${id}/following.
...
const PATH_PREFIX = __ENV.PATH_PREFIX || '/tweets';
...
export function hit() {
  ...
  const url = `${BASE_URL}${PATH_PREFIX}/${id}`;
